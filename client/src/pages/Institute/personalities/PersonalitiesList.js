        ...
        <div style={{width: "70%"}}>
            {(staff_store.staff.length !== 0) ?
                (() => {
                    let rows = []
                    const lenGroup = 3
                    const count = Math.ceil(params.filteredStaff.length / 3) * 3
                    ...
                    for (let i = 0; i < count; i += lenGroup) {
                        ...
                        rows.push(<Row style={{display: "grid", gridTemplateColumns: "1fr 1fr 1fr"}}>
                            {list.map(staffer =>
                                <div key={staffer.id}
                                     style={{width: "200px", margin: "0", backgroundColor: (params.chosenStaffer && params.chosenStaffer === staffer.id) ? "#DDDDDD" : "white"}}
                                     onClick={() => {
