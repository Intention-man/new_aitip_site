            ...
            {(staff_store.staff.length !== 0) ?
                (() => {
                    ...
                    for (let i = 0; i < linesCount; i += groupSize) {
                        ...
                        const staffer2 = groupSize > 1 ? (params.filteredStaff.length > (i + 1) ? staff_store.staff[i + 1] : undefined) : undefined
                        const staffer3 = groupSize > 2 ? (params.filteredStaff.length > (i + 2) ? staff_store.staff[i + 2] : undefined) : undefined
                        if (staffer1 !== undefined) {lastSeveralStaffId.push(staffer1.id)}
                        if (staffer2 !== undefined) lastSeveralStaffId.push(staffer2.id)
                        if (staffer3 !== undefined) lastSeveralStaffId.push(staffer3.id)

                        let list = [staffer1, staffer2, staffer3].filter(i => i !== undefined)

                        rows.push(<Row style={{display: "grid", gridTemplateColumns:
                                (groupSize === 1 ? "1fr" : (groupSize === 2 ? "1fr 1fr" : "1fr 1fr 1fr"))}}>
                            {list.map(staffer =>
                                <div key={staffer.id}
                                    ...
                                    <RoundedImg imgSrc={process.env.REACT_APP_API_URL + staffer.img}
                                         className="ava_img"
                        ...
                        rows.push(params.chosenStaffer && lastSeveralStaffId.includes(params.chosenStaffer.id) &&
