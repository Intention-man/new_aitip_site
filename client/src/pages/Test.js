...
const Test = observer(() => {
    ...
    const findSuggestions = () => {
        ...
        if (searchInput.length > 0) {
            //Firstly fill in the list with suitable page candidates (by name)
            candidates = Array.from(publicRoutes.filter(route => route.name.includes(searchInput))).map(page => ({
                type: "page",
                name: page.name
            }))
            // block candidates (by header)
            let blockCandidates = Array.from(block_store.blocks.filter(block => block.header.includes(searchInput)))
            console.log(blockCandidates)
            // if found < 10 suited blocks - continue search among lines (by text)
            if (blockCandidates.length < 10) {
                // formalize candidate-object for every block (necessary to display)
                candidates = candidates.concat(blockCandidates.map(block => ({type: "block", object: {...block}})))
                ...
                let lineCandidates = Array.from(block_store.lines.filter(line =>
                    line.hasOwnProperty("text") && line.text !== null && line.text.length > 0 && line.text.some(t => t.includes(searchInput))
                ))
                ...
                candidates = blockCandidates.map(block => ({type: "block", object: {...block}})).slice(10)
            }
        }
        console.log(candidates)
